 ...
 *
 * Soporta dos modos:
 * - Uso normal (módulo Pagos): trigger propio, cliente elegible libremente,
 *   y al confirmar muestra una pantalla con "Ver/descargar comprobante".
 * - Uso desde el módulo Clientes (open/onOpenChange controlados +
 *   clienteIdInicial fijo): el cliente ya viene elegido y NO se puede
 *   cambiar, y al confirmar se cierra directo con un toast — el
 *   comprobante NO se ofrece acá a propósito, esa acción vive solo en el
 *   módulo Pagos, para no duplicar el punto de descarga.
 */
export function NuevoPagoDialog({
  ...
  const clienteSeleccionado = clienteFijo
    ? { id: clienteIdInicial, nombre: clienteNombreInicial, saldo: clienteSaldoInicial }
  ...
  const mutation = useMutation({
    ...
    onSuccess: (pago) => {
      ...
      if (ocultarComprobante) {
        toast.success(
          `Pago de ${formatCurrency(pago.monto)} registrado. Para descargar el comprobante, andá al módulo Pagos.`
        )
        setOpen(false)
      } else {
        toast.success(`Pago de ${formatCurrency(pago.monto)} registrado`)
        ...
        {!pagoCreado ? (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                {clienteFijo
                  ? `Registrá un pago recibido de ${clienteNombreInicial}.`
                  : 'Registrá un pago recibido de un cliente.'}
              </DialogDescription>
            </DialogHeader>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              {clienteSeleccionado && clienteSeleccionado.saldo !== undefined && (
                <div className="flex items-center justify-between rounded-md bg-muted px-3 py-2 text-sm">
                  <span className="text-muted-foreground">Saldo actual</span>
                  <SaldoValor saldo={clienteSeleccionado.saldo} />
                </div>
          ...
          <>
            <DialogHeader>
              <DialogTitle>Pago registrado</DialogTitle>
              <DialogDescription>
                El pago de {pagoCreado.clienteNombre} se registró correctamente. El comprobante se
                descarga desde el módulo Pagos.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              <Button type="button" onClick={() => setOpen(false)}>
