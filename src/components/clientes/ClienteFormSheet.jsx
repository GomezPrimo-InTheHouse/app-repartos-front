...
function formDesdeCliente(cliente) {
  ...
  return {
    ...
    porcentaje_aumento: String(cliente.porcentaje_aumento ?? 0),
  ...
  const mutation = useMutation({
    mutationFn: () => {
      const payload = {
        ...
        porcentaje_aumento: Number(form.porcentaje_aumento) || 0,
      }
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      ...
      <SheetContent side="right" className="flex h-full flex-col gap-0 p-0">
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="cli-aumento">Porcentaje de aumento</Label>
            <Input id="cli-aumento" type="number" min="0" step="0.01" inputMode="decimal"
              value={form.porcentaje_aumento}
              onChange={(e) => updateField('porcentaje_aumento', e.target.value)} />
            <p className="text-xs text-muted-foreground">
              Se aplica sobre el precio de lista de cada producto al armar sus despachos. 0 = precio normal.
            </p>
          </div>
