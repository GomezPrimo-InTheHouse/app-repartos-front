...
import { ClienteComboboxFiltro } from './ClienteComboboxFiltro'
...
export function NuevoDespachoDialog({
  ...
  function handleAgregarItem() {
    ...
    if (producto.maneja_envase) {
      const saldoProducto = envasesCliente?.find((e) => e.producto_id === producto.id)?.saldo ?? 0
      const sueltoActual = Number(estado.envasesDevueltosSueltos[producto.id] || 0)
      const totalPropuesto = (itemExistente?.envases_devueltos ?? 0) + envasesDevueltos + sueltoActual

      if (totalPropuesto > saldoProducto) {
        toast.error(
          `"${producto.nombre}": el cliente tiene ${saldoProducto} envase(s) en su poder — no se pueden registrar ${totalPropuesto} entre lo cargado acá y la devolución suelta`
        )
        return
      }
    }
  ...
  function handleSubmit(event) {
    ...
    // Defensivo: handleAgregarItem ya bloquea esto al cargar el item, pero se
    // revalida acá por si en algún momento se suma otro camino que puebla
    // "items" sin pasar por esa función.
    const excedeSaldoItems = estado.items.some((item) => {
      if (!item.maneja_envase) return false
      const saldoProducto = envasesCliente?.find((e) => e.producto_id === item.producto_id)?.saldo ?? 0
      return (item.envases_devueltos ?? 0) > saldoProducto
    })
    if (excedeSaldoItems) {
      toast.error('Hay un producto con más envases devueltos de los que el cliente tiene en su poder')
      return
    }
    mutation.mutate()
  }
          ...
          {!clienteFijo && (
            <div className="flex flex-col gap-1.5">
              ...
              <ClienteComboboxFiltro
                ...
                onChange={(value) => setEstado((prev) => ({ ...prev, clienteId: value }))}
              ...
              {estado.items.length > 0 && (
                <p className="text-xs text-muted-foreground">
                  No se puede cambiar el cliente con productos ya cargados. Vaciá el carrito para elegir otro.
                </p>
              )}
            </div>
          )}
